...
const ChatContainer = () => {
  let { onlineUsers, authUser } = useContext(AuthContext);
  let { selectedUser, setSelectedUser, messages, sendMessages, getMessages } =
    useContext(ChatContext);
  let scrollEnd = useRef(null);
  ...
  let handleSendImage = async (e) => {
    ...
    reder.onloadend = async () => {
      await sendMessages({ image: reder.result });
      e.target.value = "";
    };
  ...
  return selectedUser ? (
    <div
      className="h-full overflow-y-scroll relative backdrop-blur-lg
